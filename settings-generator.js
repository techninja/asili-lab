...
class SettingsGenerator {
  constructor(options = {}) {
    ...
    this.calculationServer =
      options.calculationServer || process.env.CALCULATION_SERVER || '';
    ...
    const settings = {
      ...
      processing: {
        ...
        enableServerProcessing:
          this.mode === 'server' || this.mode === 'hybrid',
  ...
  async validateSettings() {
    ...
    try {
      ...
      console.log(`   Mode: ${settings.mode}`);
      console.log(
        `   Calculation Server: ${settings.servers?.calculation || 'Not set'}`
      );
      console.log(`   Cache Server: ${settings.servers?.cache || 'Not set'}`);
      console.log(
        `   Generated: ${settings.generated?.timestamp || 'Unknown'}`
      );
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log(
        '  ASILI_MODE         - Mode: local, server, hybrid (default: hybrid)'
      );
