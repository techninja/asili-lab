...
function summary() {
  ...
  console.log(
    `Total PGS calculations: ${stats.total_calculations.toLocaleString()}`
  );
  console.log(
    chalk.red(
  ...
  console.log(
    chalk.yellow(`High z-scores (>5σ): ${stats.high_z} (${pct(stats.high_z)}%)`)
  );
  console.log(
    chalk.yellow(
      `Low coverage (<5%): ${stats.low_coverage} (${pct(stats.low_coverage)}%)`
    )
  );
...
function extremes() {
  ...
  for (const [pgsId, pgsRows] of Object.entries(grouped)) {
    ...
    if (info) {
      console.log(
        `  Normalization: mean=${info.norm_mean?.toFixed(4)}, SD=${info.norm_sd?.toFixed(4)}`
      );
      console.log(
        `  Total variants: ${info.variants_number?.toLocaleString()}`
      );
    ...
    for (const r of pgsRows.slice(0, 3)) {
      console.log(
        chalk.red(
          `    ${r.individual_id}: z=${r.z_score.toFixed(1)}σ, raw=${r.raw_score.toFixed(4)}, coverage=${r.coverage_pct}%`
        )
      );
    }
    if (pgsRows.length > 3)
      console.log(chalk.gray(`    ... and ${pgsRows.length - 3} more`));
  }
}
...
function coverage() {
  ...
  for (const r of rows) {
    const info = qm(
      `SELECT norm_sd, variants_number FROM pgs_scores WHERE pgs_id = '${r.pgs_id}'`
    )[0];
    ...
    if (info)
      console.log(
        `  SD: ${info.norm_sd?.toFixed(6)} (${info.variants_number?.toLocaleString()} variants)`
      );
  }
}
...
function smallSDs() {
  ...
  for (const pgs of rows) {
    const usage = qr(
      `SELECT COUNT(*) as count, MAX(ABS(z_score)) as max_z FROM pgs_results WHERE pgs_id = '${pgs.pgs_id}'`
    )[0];
    console.log(chalk.yellow(`${pgs.pgs_id}:`));
    console.log(
      `  SD: ${pgs.norm_sd.toFixed(6)} (mean: ${pgs.norm_mean.toFixed(6)}) | Variants: ${pgs.variants_number?.toLocaleString()}`
    );
    console.log(`  Type: ${pgs.weight_type} | Method: ${pgs.method_name}`);
    if (usage)
      console.log(
        `  Used in ${usage.count} calculations, max |z|: ${usage.max_z?.toFixed(1)}σ`
      );
  }
}
...
function audit(filterTrait, flagsOnly) {
  ...
  for (const t of qm(
    `SELECT trait_id, editorial_name, name, emoji, trait_type, unit FROM traits`
  )) {
    traitNames.set(t.trait_id, {
      name: t.editorial_name || t.name,
      emoji: t.emoji || '',
      type: t.trait_type,
      unit: t.unit
    });
  ...
  const r2Map = new Map();
  for (const p of qm(
    `SELECT pgs_id, metric_value FROM pgs_performance WHERE metric_type IN ('R²', 'PGS R2 (no covariates)')`
  )) {
  ...
  for (const tr of traitResults) {
    const trait = traitNames.get(tr.trait_id) || {
      name: tr.trait_id,
      emoji: '',
      type: 'disease_risk'
    };
    ...
    if (bestR2 <= 0.05 && allPgs.some(p => (r2Map.get(p.pgs_id) || 0) > 0.05))
      traitFlags.push(
        '🔴 Best PGS uses default R² (0.05) but alternatives have real R²'
      );

    // Flag: runner-up has much higher R²
    if (runnerUp) {
      const runnerR2 =
        r2Map.get(runnerUp.pgs_id) || runnerUp.performance_metric || 0.05;
      if (runnerR2 > bestR2 * 2 && runnerR2 > 0.05)
        traitFlags.push(
          `🟡 Runner-up ${runnerUp.pgs_id} has ${(runnerR2 * 100).toFixed(1)}% R² vs best's ${(bestR2 * 100).toFixed(1)}%`
        );
    }
    ...
    if (runnerUp && best.quality_score - runnerUp.quality_score < 2)
      traitFlags.push(
        `🟡 Tight race: best=${best.quality_score.toFixed(1)} vs runner-up=${runnerUp.quality_score.toFixed(1)} (Δ${(best.quality_score - runnerUp.quality_score).toFixed(1)})`
      );
    ...
    // Print trait header
    const valueStr =
      tr.value != null && trait.type === 'quantitative'
        ? ` → ${tr.value.toFixed(1)} ${trait.unit || ''}`
        : '';
    const zStr =
      tr.overall_z_score != null
        ? `z=${tr.overall_z_score.toFixed(2)}σ`
        : 'z=N/A';

    console.log(chalk.cyan(`\n${'─'.repeat(80)}`));
    console.log(
      chalk.bold(
        `${trait.emoji} ${trait.name} (${tr.trait_id}) — ${tr.individual_id}`
      )
    );
    console.log(
      chalk.gray(
        `  Result: ${zStr}, P${tr.overall_percentile?.toFixed(0) || '?'}${valueStr}`
      )
    );
    ...
    if (traitFlags.length > 0) {
      ...
      allFlags.push({
        trait: `${trait.emoji} ${trait.name}`,
        individual: tr.individual_id,
        flags: traitFlags
      });
    }
    ...
    console.log(
      chalk.gray(
        '  Rank  PGS ID       QS     R²      Coverage   Genotyped  Z-score  Matched'
      )
    );
    ...
    if (allPgs.length > 5) {
      ...
      console.log(
        chalk.gray(
          `   #${allPgs.length}  ${worst.pgs_id.padEnd(12)} ${(worst.quality_score?.toFixed(1) || '?').padStart(5)}  ${(r2w * 100).toFixed(1).padStart(5)}%  ${(worst.coverage_pct + '%').padStart(8)}  ${(worst.genotyped_pct + '%').padStart(8)}   ${worst.z_score != null ? worst.z_score.toFixed(2).padStart(7) : '   N/A'}  ${worst.matched_variants?.toLocaleString().padStart(10)}`
        )
      );
    }
  }
  ...
  console.log(
    `  Flagged: ${chalk.red(flaggedTraits)} (${((flaggedTraits / totalTraits) * 100).toFixed(1)}%)`
  );
...
function validate() {
  ...
  `);
  const totalTraits =
    qr(`SELECT COUNT(DISTINCT trait_id) as n FROM trait_results`)[0]?.n || 0;
  for (const ind of indStats) {
    const pass = ind.traits === totalTraits;
    checks.push({
      name: `${ind.individual_id} has all traits`,
      pass,
      detail: `${ind.traits}/${totalTraits}`
    });
  }
  ...
  checks.push({
    name: 'Extreme z-scores (>10σ) < 5%',
    pass: extremeRate < 0.05,
    detail: `${(extremeRate * 100).toFixed(1)}% (${stats.extreme}/${stats.total})`
  });

  // Check 3: No NaN/null best PGS
  const nullBest =
    qr(`SELECT COUNT(*) as n FROM trait_results WHERE best_pgs_id IS NULL`)[0]
      ?.n || 0;
  checks.push({
    name: 'All traits have a best PGS',
    pass: nullBest === 0,
    detail: `${nullBest} missing`
  });
  ...
  `);
  checks.push({
    name: 'Cross-individual z-score spread < 10σ',
    pass: divergent.length === 0,
    detail: `${divergent.length} divergent traits`
  });

  // Check 5: No PGS with 0 expected variants
  const zeroExpected =
    qr(`SELECT COUNT(*) as n FROM pgs_results WHERE expected_variants = 0`)[0]
      ?.n || 0;
  checks.push({
  ...
  checks.push({
    name: 'Insufficient data < 20%',
    pass: insRate < 0.2,
    detail: `${(insRate * 100).toFixed(1)}%`
  });
...
const COMMANDS = {
  summary: {
    label: '📊 Summary — Overall score statistics',
    fn: () => {
      summary();
    }
  },
  extremes: {
    label: '🔴 Extremes — Analyze extreme z-scores',
    fn: () => {
      extremes();
    }
  },
  coverage: {
    label: '📉 Coverage — Low coverage PGS analysis',
    fn: () => {
      coverage();
    }
  },
  sds: {
    label: '🔬 Small SDs — Suspiciously small standard deviations',
    fn: () => {
      smallSDs();
    }
  },
  audit: {
    label: '🔍 Audit — PGS selection validation',
    fn: args => {
  ...
  validate: {
    label: '✅ Validate — Quick pass/fail checks',
    fn: () => {
      validate();
    }
  },
...
if (cmd && cmd !== '--help' && COMMANDS[cmd]) {
  ...
  if (/^[A-Z]+_\d+/.test(cmd)) {
    ...
    console.error(
      `Run ${chalk.cyan('pnpm scores --help')} for available commands`
    );
    process.exit(1);
  }
} else {
