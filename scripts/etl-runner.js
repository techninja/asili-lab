...
function checkPrerequisites() {
  ...
  } catch { missing.push('Node.js'); }
  
  try {
    execSync('python3 --version', { stdio: 'pipe' });
  } catch { missing.push('Python 3'); }
  
  try {
    execSync('duckdb --version', { stdio: 'pipe' });
  } catch { missing.push('DuckDB CLI'); }
...
function setupVenv() {
  if (!existsSync(VENV_PATH)) {
    ...
    execSync('python3 -m venv .venv', { stdio: 'inherit', cwd: path.join(__dirname, '..') });
  }
...
async function runLocal() {
  ...
  if (missing.length > 0) {
    ...
    logger.log(chalk.gray('   wget https://github.com/duckdb/duckdb/releases/download/v1.4.3/duckdb_cli-linux-amd64.zip'));
    logger.log(chalk.gray('   unzip duckdb_cli-linux-amd64.zip && sudo mv duckdb /usr/local/bin/\n'));
  ...
  logger.log(chalk.gray(`   Memory Limit: ${env.DUCKDB_MEMORY_LIMIT || '8GB (default)'}`));
  logger.log(chalk.gray(`   Threads: ${env.DUCKDB_THREADS || 'auto'}`));
  logger.log(chalk.gray(`   Parallel Batches: ${env.MAX_PARALLEL_BATCHES || '2 (default)'}\n`));
...
async function runDocker() {
  ...
  if (traitId) {
    logger.log(chalk.blue(`   Processing single trait: ${traitId}\n`));
    execSync(`docker compose run --rm -e SINGLE_TRAIT=${traitId} pipeline node etl_orchestrator.js`, { 
      stdio: 'inherit',
