...
const PHASE_PATTERNS = [
  { phase: 'init',       re: /Pipeline Starting|Loading traits|Checking prerequisites|Creating Python|Installing Python|prerequisites met|Performance Config/ },
  { phase: 'download',   re: /[Dd]ownload|cached (harmonized|PGS) file|Cache HIT|Batch \d+\/\d+: Processing \d+ files|Downloading and analyzing/ },
  { phase: 'prepare',    re: /[Pp]repare|format \(\d+ columns\)|Analyzing.*PGS files for batching|Created \d+ batches|batching/ },
  { phase: 'import',     re: /Importing |INSERT|variants \(\d+ms\)|Processing batch|batch_variants/ },
  { phase: 'duckdb',     re: /🦆|DuckDB|PRAGMA|batch \d+\/\d+ complete|Batch \d+\/\d+:.*variants$/ },
  { phase: 'merge',      re: /[Mm]erg|hierarchical|append|Concatenat/ },
  { phase: 'export',     re: /Export|COPY.*TO|standardized/ },
  { phase: 'validate',   re: /[Vv]alidat|Scanning parquet|scan-parquet|Parquet:/ },
...
function getProcessMetrics(pid) {
  const pids = [pid, ...getDescendants(pid)];
  let rssKb = 0, threads = 0;
  for (const p of pids) {
    const status = readProcFile(`/proc/${p}/status`);
    if (!status) continue;
    for (const line of status.split('\n')) {
      if (line.startsWith('VmRSS:')) rssKb += parseInt(line.split(/\s+/)[1]) || 0;
      if (line.startsWith('Threads:')) threads += parseInt(line.split(/\s+/)[1]) || 0;
    }
  }
...
async function main() {
  ...
  if (args.length === 0) {
    console.error('Usage: pnpm perf <script> [args...]\n  e.g. pnpm perf etl local EFO_0003144');
    process.exit(1);
  }
  ...
  const sampler = setInterval(() => {
    ...
    const cpuCores = deltaTicks > 0
      ? Math.min(deltaTicks / CLK_TCK / dt, NUM_CPUS)
      : 0;
  ...
  for (const sample of timeline) {
    ...
    if (!phases[p]) phases[p] = { samples: 0, cpuCoresSum: 0, peakRssMb: 0, peakThreads: 0, peakProcs: 0 };
    phases[p].samples++;
  ...
  const W = 92;
  console.log(`\n${'═'.repeat(W)}`);
  console.log(`  Profile: ${label}  |  ${totalSec}s  |  ${timeline.length} samples  |  exit ${exitCode}`);
  ...
  for (const s of timeline) {
    if (!seen.has(s.phase)) { seen.add(s.phase); phaseOrder.push(s.phase); }
  }
  ...
  console.log(`  ${'─'.repeat(W - 4)}`);
  const overallAvgCores = timeline.length > 0 ? totalCpuCoresSum / timeline.length : 0;
