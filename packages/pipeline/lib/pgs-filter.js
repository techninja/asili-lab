...
async function _validateWeights(pgsId, pgsApiClient) {
  try {
    ...
    if (
      Math.abs(min) > WEIGHT_THRESHOLDS.max_absolute ||
      Math.abs(max) > WEIGHT_THRESHOLDS.max_absolute
    ) {
      return {
        valid: false,
        reason: `Extreme weight detected: ${Math.max(Math.abs(min), Math.abs(max)).toFixed(2)}`
      };
    }
    ...
    if (variance < WEIGHT_THRESHOLDS.min_variance) {
      return {
        valid: false,
        reason: `Zero variance weights (all identical): mean=${mean.toFixed(2)}`
      };
    }
...
async function shouldExcludePGS(pgsId, scoreData, pgsApiClient = null) {
  ...
  if (scoreData.variants_number && scoreData.variants_number < 8) {
    return {
      exclude: true,
      reason: `Too few variants: ${scoreData.variants_number} (minimum 8 required)`
    };
  }
