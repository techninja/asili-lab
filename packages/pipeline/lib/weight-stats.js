...
export async function calculateWeightStatsFromFile(pgsId, filePath) {
  ...
  try {
    ...
    const described = await new Promise((resolve, reject) => {
      conn.all(
        `DESCRIBE SELECT * FROM read_csv('${filePath}', delim='\t', header=true, comment='#', all_varchar=true)`,
        (err, rows) => (err ? reject(err) : resolve(rows))
      );
    });
    const cols = described.map(r => r.column_name);
    const weightCol = cols.includes('effect_weight')
      ? 'effect_weight'
      : cols.includes('dosage_1_weight')
        ? 'dosage_1_weight'
        : cols.includes('weight')
          ? 'weight'
          : null;
    if (!weightCol) return null;

    const hasAF =
      cols.includes('allelefrequency_effect') ||
      cols.includes('effect_allele_frequency');
    if (!hasAF) {
    ...
    const afCol = cols.includes('allelefrequency_effect')
    ...
    console.error(
      `Error calculating weight stats for ${pgsId}:`,
      error.message
    );
    return null;
  } finally {
