...
describe('harmonization', () => {
  describe('detectFormat', () => {
    it('detects STANDARD_SNP (rsID + chr_name + chr_position)', () => {
      expect(detectFormat(['rsID', 'chr_name', 'chr_position', 'effect_allele', 'effect_weight']))
        .toBe(FORMAT_TYPES.STANDARD_SNP);
    });

    it('detects STANDARD_SNP_NO_RSID (chr_name + chr_position, no rsID)', () => {
      expect(detectFormat(['chr_name', 'chr_position', 'effect_allele', 'effect_weight']))
        .toBe(FORMAT_TYPES.STANDARD_SNP_NO_RSID);
    });

    it('detects DOSAGE_WEIGHTS', () => {
      expect(detectFormat(['chr_name', 'chr_position', 'effect_allele', 'other_allele', 'dosage_0_weight', 'dosage_1_weight', 'dosage_2_weight']))
        .toBe(FORMAT_TYPES.DOSAGE_WEIGHTS);
    });

    it('detects HLA_ALLELE (rsID + is_haplotype)', () => {
      expect(detectFormat(['rsID', 'effect_allele', 'effect_weight', 'is_haplotype']))
        .toBe(FORMAT_TYPES.HLA_ALLELE);
    });

    it('detects RSID_HARMONIZED (rsID + hm_chr + hm_pos)', () => {
      expect(detectFormat(['rsID', 'effect_allele', 'effect_weight', 'hm_source', 'hm_rsID', 'hm_chr', 'hm_pos', 'hm_inferOtherAllele']))
        .toBe(FORMAT_TYPES.RSID_HARMONIZED);
    });

    it('detects RSID_HARMONIZED even without hm_inferOtherAllele', () => {
      expect(detectFormat(['rsID', 'effect_allele', 'effect_weight', 'hm_chr', 'hm_pos']))
        .toBe(FORMAT_TYPES.RSID_HARMONIZED);
    });

    it('detects RSID_ONLY (rsID only, no chr/pos)', () => {
      expect(detectFormat(['rsID', 'effect_allele', 'effect_weight']))
        .toBe(FORMAT_TYPES.RSID_ONLY);
    });

    it('detects RSID_CHR (rsID + chr_name, no chr_position)', () => {
      expect(detectFormat(['rsID', 'chr_name', 'effect_allele', 'effect_weight']))
    ...
    it('prioritizes DOSAGE_WEIGHTS over STANDARD_SNP', () => {
      const cols = ['rsID', 'chr_name', 'chr_position', 'effect_allele', 'other_allele', 'dosage_0_weight', 'dosage_1_weight', 'dosage_2_weight'];
    ...
    it('prioritizes HLA_ALLELE over RSID_HARMONIZED when is_haplotype present', () => {
      expect(detectFormat(['rsID', 'effect_allele', 'effect_weight', 'is_haplotype', 'hm_chr', 'hm_pos']))
        .toBe(FORMAT_TYPES.HLA_ALLELE);
    });
  });
  ...
  describe('getColumnRef', () => {
    it('returns column index reference', () => {
      expect(getColumnRef(['rsID', 'effect_allele', 'effect_weight'], 'effect_allele')).toBe('column1');
    });
  ...
  describe('generateColumnDefinitions', () => {
    it('generates DuckDB column defs for CSV reading', () => {
      const defs = generateColumnDefinitions(['rsID', 'effect_allele', 'effect_weight']);
      expect(defs).toBe("'column0': 'VARCHAR', 'column1': 'VARCHAR', 'column2': 'VARCHAR'");
    });
  });

  describe('generateColumnExpressions', () => {
    describe('RSID_HARMONIZED', () => {
      const cols = ['rsID', 'effect_allele', 'effect_weight', 'hm_source', 'hm_rsID', 'hm_chr', 'hm_pos', 'hm_inferOtherAllele'];

      it('builds chr:pos:allele:other variant_id from hm_ columns', () => {
        const exprs = generateColumnExpressions(FORMAT_TYPES.RSID_HARMONIZED, cols);
      ...
      it('falls back gracefully without hm_inferOtherAllele', () => {
        const minCols = ['rsID', 'effect_allele', 'effect_weight', 'hm_chr', 'hm_pos'];
        const exprs = generateColumnExpressions(FORMAT_TYPES.RSID_HARMONIZED, minCols);
      ...
      it('prefers other_allele over hm_inferOtherAllele when both present', () => {
        const bothCols = ['rsID', 'effect_allele', 'other_allele', 'effect_weight', 'hm_chr', 'hm_pos', 'hm_inferOtherAllele'];
        const exprs = generateColumnExpressions(FORMAT_TYPES.RSID_HARMONIZED, bothCols);
    ...
    describe('STANDARD_SNP', () => {
      it('builds chr:pos:allele variant_id', () => {
        const cols = ['rsID', 'chr_name', 'chr_position', 'effect_allele', 'effect_weight'];
        const exprs = generateColumnExpressions(FORMAT_TYPES.STANDARD_SNP, cols);
      ...
      it('includes other_allele in variant_id when present', () => {
        const cols = ['rsID', 'chr_name', 'chr_position', 'effect_allele', 'other_allele', 'effect_weight'];
        const exprs = generateColumnExpressions(FORMAT_TYPES.STANDARD_SNP, cols);
    ...
    describe('DOSAGE_WEIGHTS', () => {
      it('uses dosage_1_weight as effect_weight', () => {
        const cols = ['chr_name', 'chr_position', 'effect_allele', 'other_allele', 'dosage_0_weight', 'dosage_1_weight', 'dosage_2_weight'];
        const exprs = generateColumnExpressions(FORMAT_TYPES.DOSAGE_WEIGHTS, cols);
